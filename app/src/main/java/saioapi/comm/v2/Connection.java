...
abstract class Connection {
    private final String TAG = "CM-Connection";

    private final int MAX_PACKET = 1024;
    private final int MAX_READ_PACKET = 1024;
    private final int MAX_RETRIES = 3;
    private final int SEND_COMMAND_SUCCESS = 4;
    ...
    public int connect(int protocol) {
        if ((protocol == ComManager.PROTOCOL_RAW_DATA) || (protocol == ComManager.PROTOCOL_XAC_VNG)) {
            mProtocol = protocol;
            return 0;
        }else{
            Log.i(TAG, "Invalid protocol parameter. Usb device only supports Raw data and VNG protocols");
            return ComManager.ERR_INVALID_PARAM;
        }
    }
