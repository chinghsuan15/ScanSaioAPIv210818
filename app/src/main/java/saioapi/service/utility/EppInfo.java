...
public class EppInfo implements Serializable {
    ...
    private static final String BUNDLE_KEY_ERR = "comm_err";
     ...
     *
     * @param b The mapping from String values to various Parcelable types.
     * @param iErr The key's slot description.
     */
    public static void setCommErrorCode(Bundle b, int iErr)
    {
        if(null == b)
            return;

        b.putInt(BUNDLE_KEY_ERR, iErr);
        // b.putSerializable(BUNDLE_KEY_ERR, iErr);
    }
     ...
     * @return  Return the value is key's check value. If the function succeeds else return null.
     */
    public static int getCommErrorCode(Bundle b){
        if(null == b)
            return 0;
        else
            return b.getInt(BUNDLE_KEY_ERR);
    }
